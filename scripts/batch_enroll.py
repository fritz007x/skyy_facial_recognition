...
class BatchEnrollmentClient:
    ...
    def get_image_files_for_lfw(self) -> Dict[str, List[Path]]:
        """
        Get images organized by person folder for LFW-style datasets.

        Returns:
            Dict mapping person name (folder name) to list of image paths
            Example: {"Aaron_Eckhart": [path1.jpg, path2.jpg], ...}
        """
        if not self.enrollment_dir.exists():
            raise FileNotFoundError(
                f"Enrollment directory not found: {self.enrollment_dir}\n\n"
                f"Create the folder and add person subfolders:\n"
                f"  mkdir {self.enrollment_dir}\n"
                f"  mkdir {self.enrollment_dir}/PersonName1\n"
                f"  mkdir {self.enrollment_dir}/PersonName2"
            )
        ...
        for image_path in self.enrollment_dir.rglob('*'):
            # Check if file matches supported extensions
            if not image_path.is_file():
                continue

            if not any(image_path.match(ext) for ext in supported_extensions):
                continue

            # Use parent folder name as person name
            person_folder = image_path.parent.name

            # Skip if in root directory (not in a person folder)
            if image_path.parent == self.enrollment_dir:
                continue

            if person_folder not in person_images:
                person_images[person_folder] = []

            person_images[person_folder].append(image_path)

        if not person_images:
            raise FileNotFoundError(
                f"No person folders with images found in {self.enrollment_dir}\n\n"
                f"Expected structure:\n"
                f"  {self.enrollment_dir}/\n"
                f"    PersonName1/\n"
                f"      image1.jpg\n"
                f"      image2.jpg\n"
                f"    PersonName2/\n"
        ...
        # Sort images per person for consistency
        for person in person_images:
            person_images[person].sort()

        return person_images

    async def enroll_all_lfw(self):
        """Main enrollment process for LFW-style datasets (folder-based structure)."""
        print("=" * 80)
        print("BATCH ENROLLMENT - LFW Dataset Mode")
        print("=" * 80)
        print(f"\nEnrollment folder: {self.enrollment_dir}")
        print(f"Mode: LFW (person subfolders)")
        ...
        # Get person folders and their images
        try:
            person_images = self.get_image_files_for_lfw()
            self.total_images = sum(len(images) for images in person_images.values())
            print(f"[Found] {len(person_images)} person folders with {self.total_images} total images\n")
        ...
        async with self.get_mcp_session() as (read, write):
            async with ClientSession(read, write) as session:
                ...
                # Enroll each person (one enrollment per person, using first image)
                print("-" * 80)
                print("ENROLLING PEOPLE FROM LFW FOLDERS")
                print("-" * 80)
                ...
                for person_name in sorted(person_images.keys()):
                    ...
                    # Use first image for enrollment (consistent, deterministic)
                    ...
                    if self.skip_existing and normalized_name in self.existing_users:
                        print(f"[{person_idx}/{len(person_images)}] [SKIP] {person_name} - Already enrolled ({image_count} images available)")
                        self.skip_count += 1
                        continue

                    print(f"[{person_idx}/{len(person_images)}] Enrolling {person_name}...", end=' ', flush=True)
                    if image_count > 1:
                        print(f"(using 1 of {image_count} images)", end=' ', flush=True)

                    # Enroll via MCP (using first image only)
                    result = await self.enroll_single_user(session, image_to_use, person_name)
                ...
                # Print summary
                await self.print_summary_lfw(session, person_images)

    async def print_summary_lfw(self, session: ClientSession, person_images: Dict[str, List[Path]]):
        """Print enrollment summary for LFW mode."""
        print("\n" + "=" * 80)
        print("ENROLLMENT SUMMARY (LFW MODE)")
        print("=" * 80)
        print(f"Person folders processed: {len(person_images)}")
        print(f"Total images available: {sum(len(images) for images in person_images.values())}")
        ...
        if self.success_count > 0 or self.queued_count > 0:
            print(f"\n[OK] LFW enrollment completed successfully!")

            if self.queued_count > 0:
                print(f"\n[WARNING] {self.queued_count} registrations are queued")
            ...
            print("  python src/utils/camera_utils.py")
            print("\nOr run automated tests:")
            print("  python tests/integration/test_mcp_client.py")
            print("=" * 80)
    ...
    async def print_summary(self, session: ClientSession):
        ...
        # Next steps
        if self.success_count > 0 or self.queued_count > 0:
            print(f"\n[OK] Enrollment completed successfully!")
...
async def main():
    ...
    parser.add_argument(
        '--lfw',
        action='store_true',
        help='Use LFW dataset mode: images organized in person folders, uses folder names as person identities'
    )

    args = parser.parse_args()

    # Resolve enrollment directory with user-friendly defaults
    if args.enrollment_dir is None:
        # Try common locations
        possible_paths = [
            Path.home() / "Documents" / "facial_recognition" / "enrollment",
            Path.home() / "Downloads" / "facial_enrollment",
            Path(__file__).parent.parent / "test_dataset" / "enrollment",
        ]
        ...
        for path in possible_paths:
            if path.exists():
                ...
                print(f"[Info] Using enrollment directory: {path}\n")
                break

        if enrollment_dir is None:
            print("[Error] No enrollment directory specified and no default found")
            print("[Error] Please create enrollment folder or provide path")
            print("[Error] Example: python batch_enroll.py C:\\path\\to\\enrollment")
            sys.exit(1)

        args.enrollment_dir = enrollment_dir
